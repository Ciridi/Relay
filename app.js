...
function render(){const h=location.hash.replace(/^#\/?/,"");if(h.startsWith("project/")){state.currentProjectId=decodeURIComponent(h.split("/")[1]||"");renderBuilder()}else if(h==="projects")renderProjects();else renderHome();main.focus({preventScroll:true})}
...
function projectCard(p){return`<article class="project-card" data-open-project="${esc(p.id)}" tabindex="0" role="button"><img class="thumb" src="${esc(p.image_url||p.image||FALLBACK_IMAGE)}" alt=""><div><h3>${esc(p.name)}</h3><div class="meta">Last modified ${esc(dateText(p.updated_at||p.updatedAt))}</div><div class="project-hover">${esc(p.description||"No description yet.")}<br>${esc(dateText(p.start_date||p.startDate))} · ${p.part_count??p.parts?.length??0} modifications</div></div><div class="project-total">${money(p.total_cost??total(p))}</div></article>`}
...
function openProjectDialog(p=null){$("#projectDialogTitle").textContent=p?"Edit project":"New project";$("#projectId").value=p?.id||"";$("#projectName").value=p?.name||"";$("#projectDescription").value=p?.description||"";$("#projectStartDate").value=p?.start_date||new Date().toISOString().slice(0,10);$("#projectImage").value=p?.image_url||"";$("#deleteProjectButton").hidden=!p;$("#projectDialog").showModal()}
async function saveProject(e){e.preventDefault();const id=$("#projectId").value,name=$("#projectName").value.trim();if(!name)return;const payload={name,description:$("#projectDescription").value.trim(),start_date:$("#projectStartDate").value||null,image_url:$("#projectImage").value.trim()||null};if(payload.image_url)try{new URL(payload.image_url)}catch{notify("Please enter a valid image URL.");return}
